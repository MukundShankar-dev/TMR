...
def retrieval(newcfg: DictConfig) -> None:
    ...
    main_df = pd.read_csv("embeddings.csv")
    all_dtw_scores = torch.load("dtw_scores.pt")
    ...
    save_dir = os.path.join(run_dir, "contrastive_metrics_2")
    ...
    cfg.model.log_wandb = False

    import pytorch_lightning as pl
    import numpy as np
    from hydra.utils import instantiate
    from src.load import load_model_from_cfg
    from src.model.dtw_metrics import all_contrastive_metrics
    ...
    for protocol in protocols:
        ...
        if protocol == "guo":
            ...
            for x in result:
                ...
                metrics, t2m_retrievals, m2t_retrievals = all_contrastive_metrics(sim_matrix, dataset.keyids, main_df, all_dtw_scores, rounding=None)
            ...
            metrics, t2m_retrievals, m2t_retrievals = all_contrastive_metrics(sim_matrix, dataset.keyids, main_df, all_dtw_scores, emb, threshold=threshold)

            # breakpoint()
        # with open(f"{save_dir}/{protocol_name}_keyid_metrics.json", 'w') as file:
            # json.dump(keyid_metrics, file)

        dataset.keyids = np.array(dataset.keyids)
        t2m_retrieval_dict = {}
